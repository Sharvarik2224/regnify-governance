...
const run = async () => {
  ...
  await db.collection(mongoMetricsOneCollection).createIndex({ employee_email: 1 }, { unique: true });
  await db.collection(mongoMetricsLegacyCollection).createIndex({ employee_email: 1 }, { unique: true });
